...
整理前测试文件数 242，默认收集 1,716（总计 1,726，10 个按标记排除）。分类移动本身没有减少测试；发布脚本、图标、路径安全、原生启动门和 Qt 生命周期回归新增后，测试文件为 246，默认收集 1,745（总计 1,755，10 个按标记排除）。正式默认回归结果为 `1745 passed, 10 deselected in 257.59s`。
