...
| 测试临时文件 | `.test-artifacts`、根目录 `.pytest_tmp_*` | 只归档或清理通过边界检查且 ACL 允许的目标 |
...
另有 301 个旧 `.pytest_tmp_*` 目录最初因 Windows ACL 返回 `WinError 5`。用户明确授权后，仅对 `D:\Trading` 的这些直接子目录递归执行 `takeown` 和 `icacls`；脚本要求目录名严格匹配 `.pytest_tmp_*`、父目录必须是工作区根目录，并拒绝符号链接和 Windows junction。权限修复完成后，目录原样移动到 `.test-artifacts/legacy-root-pytest`，没有删除内容。新的 pytest 运行不再复用固定目录，而是使用 `.test-artifacts/pytest-tmp-run-<pid>-<uuid>`。
...
整理前测试文件数 242，默认收集 1,716（总计 1,726，10 个按标记排除）。分类移动本身没有减少测试；发布脚本、图标、路径安全、原生启动门、Qt 生命周期和 CI PowerShell 兼容性回归新增后，测试文件为 246。发布前缺陷修复再增加字体、主题传播、绩效入口、冻结程序数据路径和启动预算边界回归，最终默认收集 1,752（总计 1,763，11 个按标记排除）。根目录归档后的最终默认回归结果为 `1752 passed, 11 deselected in 235.43s`。

## 发布审查修复

- clean-release 只允许替换带本项目生成标记的旧输出，拒绝删除未知目录。
- clean-release、正式 manifest、ZIP 归档、依赖裁剪和测试产物清理均拒绝跟随符号链接或 Windows junction。
- 测试分类后的 Qt native 子进程路径已改为 `tests/performance/test_qt_layout_stress.py`；最终生命周期独立回归 `1 passed in 67.54s`，根目录归档后的完整性能集为 `9 passed, 1754 deselected in 278.42s`。
...
- `.test-artifacts/legacy-root-pytest`：301 个旧 pytest 临时目录的原样归档，仅用于本地审计，继续由 Git 和发布包排除。
...
初次检查时，内容枚举、ACL 读取和移动均被 Windows 拒绝。用户随后明确授权仅修复这些测试残留的权限。管理员脚本在执行前验证了 301 个目录全部是 `D:\Trading` 的直接子目录、名称严格匹配 `.pytest_tmp_*`，并且没有符号链接或 Windows junction；权限修复结果为 301 成功、0 失败。

归档前清点得到 87,166 个文件、17,468 个内部子目录、6,835,903,685 字节；归档后文件数和总字节数完全一致。归档目录为 `D:\Trading\.test-artifacts\legacy-root-pytest`，包含 301 个顶层目录，递归扫描仍为 0 个链接或 junction。根目录 `.pytest_tmp_*` 数量为 0。没有删除测试产物，也没有修改 `quant_collector_app/data`、`data`、`backups`、`Backup` 或 `exports`。

上表是发布整理开始时的受保护数据基线；用户随后继续使用应用，因此主数据库哈希在本次归档开始前已发生变化。归档及全部回归结束后，主数据库仍为 189,095,936 字节，修改时间保持在归档开始前的 `2026-07-21 17:34:45 +08:00`，SHA-256 为 `2A671DF7FD1174E30C6A064726E520947710445B49E1864D54A1D03E78853348`。归档命令的源路径只包含根目录 `.pytest_tmp_*`，目标路径只包含 `.test-artifacts/legacy-root-pytest`。

归档完成后的验证结果：pytest 收集 `1752/1763`，11 个按标记排除；默认回归 `1752 passed, 11 deselected in 235.43s`；性能回归 `9 passed, 1754 deselected in 278.42s`；发布测试 `128 passed in 23.44s`；core/gui/all 自检全部返回 `status: ok`。这些运行均未在项目根目录重新生成 `.pytest_tmp_*`。
