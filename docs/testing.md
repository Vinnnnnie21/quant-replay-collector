...
python scripts/clean_release.py --output dist/QuantReplayCollector-v1.5.2-Clean
python scripts/check_release_clean.py dist/QuantReplayCollector-v1.5.2-Clean
```

Default pytest cache and temporary output are kept below `.test-artifacts/` so test runs do not create many directories at the repository root. The directory is local-only and excluded from release packages.
