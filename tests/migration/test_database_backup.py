...
def test_integrity_check_requires_all_current_research_tables(tmp_path):
    storage = StorageManager(tmp_path / "incomplete_current.db")
    with sqlite3.connect(storage.db_path) as conn:
        conn.execute("DROP TABLE exit_outcome_matches")
    ...
    assert report["status"] == "warning"
    assert report["migration_status"] == "incomplete_schema"
    assert report["missing_required_tables"] == ["exit_outcome_matches"]
    assert report["protected_table_counts"]["exit_outcome_matches"] is None
