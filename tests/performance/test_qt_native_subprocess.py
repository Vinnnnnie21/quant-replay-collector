...
_NATIVE_PROCESS_WATCHDOG_SECONDS = 300
        ...
        try:
            run = subprocess.run(
                [
                    ...
                    "-m",
                    "qt_native_inner",
                    "tests/performance/test_qt_layout_stress.py",
                    "-q",
                    "-s",
                    "--basetemp",
                ...
                stdout=stdout_file,
                stderr=stderr_file,
                # Each worker stop has its own two-second assertion in the
                # child test.  This outer timeout is only a dead-process
                # watchdog, so leave enough headroom for Windows scheduling
                # while 200 complete Qt object trees are created and deleted.
                timeout=_NATIVE_PROCESS_WATCHDOG_SECONDS,
            )
