...
def test_app_and_window_use_the_stable_multisize_icon_across_themes():
    app = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])
...
def test_windows_taskbar_identity_is_stable_and_configured_before_qapplication(monkeypatch):
    calls: list[str] = []
    monkeypatch.setattr(
        "app_icon._set_current_process_app_user_model_id",
        lambda value: calls.append(value) or 0,
    )

    assert configure_windows_taskbar_identity(platform_name="win32") is True
    assert calls == [WINDOWS_APP_USER_MODEL_ID]
    assert configure_windows_taskbar_identity(platform_name="linux") is False

    main_source = (APP_DIR / "main_app.py").read_text(encoding="utf-8")
    identity_call = main_source.index("configure_windows_taskbar_identity()")
    application_create = main_source.index("QtWidgets.QApplication(sys.argv)")
    assert identity_call < application_create
