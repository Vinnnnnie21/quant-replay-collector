...
def test_explicit_runtime_root_isolated_native_validation_from_package(tmp_path):
    package = tmp_path / "release" / "QRC"
    runtime = tmp_path / "native-validation-runtime"
    ...
    assert paths.root_dir == runtime.resolve()
    assert paths.data_dir == runtime.resolve() / "data"
    assert paths.log_dir == runtime.resolve() / "logs"
    assert paths.backup_dir == runtime.resolve() / "backups"
