...
from tests.storage.test_storage_trade_flow import (
    INTERVAL,
    NOW,
    SESSION_ID,
    SYMBOL,
...
def test_existing_session_export_remains_compatible_with_ancillary_kline_storage(
    tmp_path,
):
    storage = make_storage(tmp_path)
    insert_session(storage)
    ...
    assert manifest["row_counts"]["trades"] == 1
    assert manifest["row_counts"]["trade_events"] == 2
    assert (export_dir / "event_windows_long.csv").is_file()
    assert (export_dir / "sessions.csv").is_file()
