...
from services.ui_message_localizer import (
    localize_worker_message,
    sanitize_worker_error_detail,
)
...
def test_research_worker_errors_do_not_expose_python_types_or_internal_codes():
    assert sanitize_worker_error_detail("ConnectionError: offline") == "offline"
    assert sanitize_worker_error_detail("SNAPSHOT_NOT_FOUND: 引用不存在") == "引用不存在"
    assert sanitize_worker_error_detail("plain diagnostic") == "plain diagnostic"
