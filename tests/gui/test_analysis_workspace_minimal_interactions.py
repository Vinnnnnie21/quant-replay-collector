...
def test_parent_owned_graphics_view_detaches_scene_only_after_native_close(
    ...
    view = pg.GraphicsView(parent)
    scene = view.scene()
    assert scene is not None
    scene_during_close = []

    def close_with_leave_event(watched):
        scene_during_close.append(watched.scene())
        watched.leaveEvent(QtCore.QEvent(QtCore.QEvent.Leave))
        return True

    monkeypatch.setattr(QtWidgets.QGraphicsView, "close", close_with_leave_event)

    close_parent_owned_graphics_view(view)

    assert scene_during_close == [scene]
