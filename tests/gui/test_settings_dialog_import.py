...
def test_full_history_ancillary_backfill_requires_explicit_maintenance_click(
    monkeypatch,
):
    os.environ.setdefault("QT_QPA_PLATFORM", "offscreen")
    QtWidgets = pytest.importorskip("PySide6.QtWidgets")
    ...
    host.current_language = "zh_CN"
    starts = []
    host.start_full_history_ancillary_backfill = lambda: starts.append(True)
    monkeypatch.setattr(
        settings_dialog,
        "load_app_settings",
        lambda: {"language": "zh_CN"},
    )

    dialog = SettingsDialog(host)
    try:
        assert starts == []
        assert dialog.btnBackfillFullHistory.text() == "补齐本地全量历史附加字段"

        dialog.btnBackfillFullHistory.click()
