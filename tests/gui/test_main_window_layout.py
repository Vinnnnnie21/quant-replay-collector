...
def test_main_window_layout_builds_existing_primary_widgets():
    ...
    assert not hasattr(host, "eventResearchTabs")
    assert not hasattr(host, "eventResearchStack")
    assert host.bottomTabs.tabText(
        host.bottomTabs.indexOf(host.decisionResearchRedirect)
    ) in {"事件研究已迁移", "Event Study Moved"}
    assert host.btnOpenDecisionResearch.property("role") == "primaryButton"
    ...
    assert host.headerTitleLabel.text() == f"Quant Replay Collector v{APP_VERSION}"
    ...
    assert host.decisionResearchRedirect is not None
...
def test_layout_save_keeps_expanded_panel_preference_when_replay_page_is_hidden(
    monkeypatch,
):
    import main_app
    ...
    try:
        ...
        host.show()
        app.processEvents()
        assert host.btnToggleRightPanel.isChecked()
        analysis_page = QtWidgets.QWidget()
        host.workspaceStack.addWidget(analysis_page)
        host.workspaceStack.setCurrentWidget(analysis_page)
        app.processEvents()
        assert not host.rightTabs.isVisibleTo(host)

        main_app.MainWindow._save_layout_preferences(host)

        assert saved["right_panel_visible"] is True
        assert saved["bottom_panel_visible"] is True
    finally:
        _close_layout_host(host, app)
