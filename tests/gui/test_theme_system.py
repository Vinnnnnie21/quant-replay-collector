...
@pytest.mark.skipif(os.name != "nt", reason="v1.6 desktop runtime is Windows")
def test_windows_ui_font_bootstrap_provides_chinese_glyphs():
    from PySide6 import QtGui, QtWidgets

    app = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])
    family = ensure_ui_font_support(app)

    assert family is not None
    metrics = QtGui.QFontMetrics(QtGui.QFont(family))
    assert all(metrics.inFontUcs4(ord(character)) for character in "决策研究")
    assert UI_FONT_FAMILY.index(f'"{family}"') < UI_FONT_FAMILY.index('"Inter"')
...
def test_qt_palette_and_chart_items_use_reference_colors(monkeypatch):
    ...
    from tests.gui.test_main_window_layout import _LayoutHost, _close_layout_host
...
def test_main_window_uses_stable_window_icon_and_theme_header_logo(monkeypatch):
    os.environ.setdefault("QT_QPA_PLATFORM", "offscreen")
    QtWidgets = pytest.importorskip("PySide6.QtWidgets")
    pytest.importorskip("pyqtgraph")
    ...
    assert light_corner == dark_corner
