...
def test_legacy_research_actions_redirect_without_starting_exports(
    tmp_path,
    monkeypatch,
):
    ...
    for action in (
        dialog.run_research_analysis,
        dialog.export_research_pack,
        dialog.run_entry_logic_report,
        dialog.export_entry_logic_report,
    ):
        dialog.tabs.setCurrentWidget(dialog.researchTab)
        action()
        assert dialog.tabs.currentWidget() is dialog.decisionResearchTab

    assert host.export_calls == []
...
def test_entry_logic_export_output_is_loaded_into_summary_and_queue(tmp_path):
    ...
    assert "开仓：2" in dialog.entryLogicSummary.text()
    assert "拒绝开仓：1" in dialog.entryLogicSummary.text()
    assert "未标注：4" in dialog.entryLogicSummary.text()
