...
from quant_collector_app.research.market_episodes import (
...
def test_experiment_manifest_keeps_the_immutable_episode_grouping_reference(tmp_path):
    features = _features()
    labeled_ids = tuple(
        _annotations()
        .loc[
            _annotations()["human_decision"].isin(["ENTRY", "REJECT"]),
            "observation_id",
        ]
        .astype(str)
    )
        ...
        tuple(
            ResearchSampleWindow(
                ...
                feature_window=TimeRange(
                    start + pd.Timedelta(minutes=index * 10),
                    start + pd.Timedelta(minutes=index * 10 + 1),
                ),
                outcome_window=TimeRange(
                    start + pd.Timedelta(minutes=index * 10 + 1),
                    start + pd.Timedelta(minutes=index * 10 + 2),
                ),
            )
            for index, sample_id in enumerate(labeled_ids)
        ),
        created_at=start + pd.Timedelta(days=1),
    )
    ...
    assert result["split_summary"]["episode_grouping_version_id"] == grouping.grouping_version_id
    assert manifest["episode_grouping_version_id"] == grouping.grouping_version_id
    assert manifest["split_summary"]["episode_grouping_version_id"] == grouping.grouping_version_id
