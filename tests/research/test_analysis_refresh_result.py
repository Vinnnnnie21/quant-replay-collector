...
def test_live_analysis_refresh_does_not_run_legacy_event_study_path():
    legacy_calls: list[bool] = []
    snapshot = AnalysisRefreshSnapshot(
        events=[{"event_id": "evt_legacy"}],
        features=[{"event_id": "evt_legacy"}],
        trades=[],
        equity_rows=[],
    ...
    result = build_analysis_refresh_result(
        snapshot,
        build_event_study_fn=lambda _events, _features: (
            legacy_calls.append(True) or pd.DataFrame([{"legacy": True}])
        ),
...
def test_analysis_refresh_builds_trade_equity_in_worker_when_market_frame_is_absent():
        ...
        trades=[
            {
                ...
                "exit_bar_time_bjt": "2026-01-01T00:01:00+08:00",
                "net_pnl_quote": 20.0,
    ...
    assert result.equity_rows[0]["created_at"] == "2026-01-01T00:01:00+08:00"
    assert result.performance_workspace.markers == ()
...
def test_analysis_refresh_progress_uses_snapshot_language():
    ...
    assert progress_messages == ["正在准备研究样本…", "正在计算绩效统计…"]
