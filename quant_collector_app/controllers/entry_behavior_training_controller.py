...
try:
    from workers.entry_behavior_training_worker import (
...
class EntryBehaviorTrainingController(QtCore.QObject):
    ...
    def __init__(
        ...
        service: Any,
        *,
        worker_factory: Callable[..., Any] = EntryBehaviorTrainingWorker,
    ...
    def _start(self, revision: int, request: Any) -> None:
        if self._lifecycle is not None and not self._lifecycle.start(
            "entry_behavior_training",
            request_stop=self.cancel,
        ):
            self.failed.emit(
                "entry behavior training cannot start while another task is active"
            )
            return
        try:
    ...
    def _handle_start_failure(
        self,
        event: EntryBehaviorTrainingFailure,
    ) -> None:
        ...
        if terminal is not None:
            ...
            if self._lifecycle is not None:
                if kind == "failed" and revision == self._revision:
                    ...
                    self._lifecycle.complete("entry_behavior_training")
...
__all__ = ["EntryBehaviorTrainingController"]
